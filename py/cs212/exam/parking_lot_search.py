...
把'* *'代表的车，移动到@处。'A A'或'O' 表示车，只能顺车的停放方向移动。
                               'O'
        ...
        if c not in '|@':  # Walls and goals can't move
            ...
            for (d, start) in [(diff, max(sqs)), (-diff, min(sqs))]:
                for i in range(1, N - 2):
                    ...
                    results[update(state, c, tuple(q + d * i for q in sqs))] = (
                        c, d * i)
    return results
...
def show(state, N=N):
    ...
    # Initialize and fill in the board.
    board = convert2board(state, N)
    # Now print it out
    # for i, s in enumerate(board):
    #     print(s, end=' ')
    #     if i % N == N - 1: print()
    for l in range(N):
        print(' '.join(board[l * N:l * N + N]))
...
def show_path(path, N=N, MAX=3):
    states = path[0::2]
    actions = path[1::2]
    boards = [convert2board(s, N) for s in states]
    # show state1 --action1--> state2 in multi-line
    for sub in range(int((len(boards) + MAX - 1) / MAX)):
        sub_boards = boards[sub * 3:sub * 3 + MAX]
        for l in range(N):
            for s, b in enumerate(sub_boards):
                s = sub * 3 + s
                print(' '.join(b[l * N:l * N + N]), end=' ')
                if s < len(actions):
                    if l == int(N / 2):
                        c, m = actions[s][0], actions[s][1]
                        m = m / N if m % N == 0 else m
                        print('%s %2d -->' % (c, m), end=' ')
                    else:
                        print(' ' * 9, end='')
            print()
