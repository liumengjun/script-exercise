...
# 代码是从`python`原库中`copy`过来的，此处修改确保只`import`当前路径下的文件, 即是修改目录文件名也没问题。
demo_dir_parent = os.path.dirname(demo_dir)
demo_dir_name = os.path.basename(demo_dir)
sys.path.insert(0, demo_dir_parent)
__import__(demo_dir_name)
about_turtledemo = sys.modules[demo_dir_name].__doc__
# print(about_turtledemo)
...
class DemoWindow(object):
    ...
    def loadfile(self, filename):
        ...
        modname = demo_dir_name + '.' + filename
        __import__(modname)
